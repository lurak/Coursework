...
CULTURAL_RESORTS.create("files\\data.json", "cultural resorts")
SEA_RESORTS.create("files\\data.json", "sea resorts")
SKI_RESORTS.create("files\\data.json", "ski resorts")
RESORTS = {"cultural resorts": CULTURAL_RESORTS, "sea resorts": SEA_RESORTS, "ski resorts": SKI_RESORTS}
PLACES = PlaceInfo(1257)
i = 0
with open("files\\ids.txt", encoding='utf-8') as f:
    while True:
        line = f.readline()
        if not line:
            break
